...
manual_shadowsofalmia_letsgolilly_location = {}
manual_shadowsofalmia_letsgolilly_location.__index = manual_shadowsofalmia_letsgolilly_location
...
function manual_shadowsofalmia_letsgolilly_location.new(name)
    local self = setmetatable({}, manual_shadowsofalmia_letsgolilly_location)
    if name then
...
function manual_shadowsofalmia_letsgolilly_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = manual_shadowsofalmia_letsgolilly_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function manual_shadowsofalmia_letsgolilly_location:connect_two_ways(exit, rule)
...
function manual_shadowsofalmia_letsgolilly_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function manual_shadowsofalmia_letsgolilly_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function manual_shadowsofalmia_letsgolilly_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function manual_shadowsofalmia_letsgolilly_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function manual_shadowsofalmia_letsgolilly_location:accessibility()
...
-- 
function manual_shadowsofalmia_letsgolilly_location:discover(accessibility, keys)
...
entry_point = manual_shadowsofalmia_letsgolilly_location.new("entry_point")
